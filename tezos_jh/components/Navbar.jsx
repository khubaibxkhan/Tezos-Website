// "use client";
// import React, { useState, useEffect } from "react";
// import { Menu, X } from "lucide-react"; // Icons for mobile menu toggle

// export const Navbar = () => {
//   // State for checking if the page is scrolled
//   const [scrolled, setScrolled] = useState(false);

//   // State for opening/closing the mobile menu
//   const [menuOpen, setMenuOpen] = useState(false);

//   // Add scroll effect listener
//   useEffect(() => {
//     const handleScroll = () => setScrolled(window.scrollY > 20);
//     window.addEventListener("scroll", handleScroll);
//     return () => window.removeEventListener("scroll", handleScroll);
//   }, []);

//   // Function to handle resume download (Join Us button)
//   const handleResumeDownload = () => {
//     const link = document.createElement("a");
//     link.href = "/resume.pdf";
//     link.download = "KhubaibAhmadKhan_Resume.pdf";
//     link.click();
//   };

//   return (
//     <nav
//       className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 bg-transparent ${
//         scrolled ? "backdrop-blur-md shadow-md" : "" // Add blur + shadow after scroll
//       }`}
//     >
//       <div className="max-w-7xl mx-auto px-6 flex justify-between items-center h-16">
//         {/* ---------- Logo Section ---------- */}
//         <div className="text-3xl md:text-4xl lg:text-5xl font-light m-0">
//           <img
//             src="/logo.png" // Replace with your logo path
//             alt="Logo"
//             className="h-8 md:h-10 lg:h-12 w-auto"
//           />
//         </div>

//         {/* ---------- Desktop Menu ---------- */}
//         <div className="hidden md:flex space-x-10 text-white tracking-wider">
//           {/* Anchor link to Home section */}
//           <a
//             href="#home"
//             className="text-base tracking-wider transition-colors hover:text-purple-400"
//           >
//             Home
//           </a>

//           {/* ✅ Members link opens in a NEW TAB (CHANGED) */}
//           <a
//             href="/members"
//             target="_blank" // Opens in a new tab
//             rel="noopener noreferrer" // Security best practice
//             className="text-base tracking-wider transition-colors hover:text-purple-400"
//           >
//             Members
//           </a>

//           {/* Other anchor links (still scroll on same page) */}
//           <a
//             href="#skills"
//             className="text-base tracking-wider transition-colors hover:text-purple-400"
//           >
//             Events
//           </a>
//           <a
//             href="#projects"
//             className="text-base tracking-wider transition-colors hover:text-purple-400"
//           >
//             Blogs
//           </a>
//           <a
//             href="#connect"
//             className="text-base tracking-wider transition-colors hover:text-purple-400"
//           >
//             Connect
//           </a>
//         </div>

//         {/* ---------- Join Us Button (Desktop) ---------- */}
//         <div className="hidden md:block">
//           <button
//             onClick={handleResumeDownload}
//             className="bg-[#a7a7a7] text-black py-3 px-8 rounded-full border-none font-medium transition-all duration-500 hover:bg-white"
//           >
//             Join Us
//           </button>
//         </div>

//         {/* ---------- Mobile Menu Button (hamburger/close) ---------- */}
//         <div className="md:hidden text-white hover:text-purple-300">
//           <button onClick={() => setMenuOpen(!menuOpen)}>
//             {menuOpen ? <X size={28} /> : <Menu size={28} />}{" "}
//             {/* Toggle between icons */}
//           </button>
//         </div>
//       </div>

//       {/* ---------- Mobile Menu Dropdown ---------- */}
//       {menuOpen && (
//         <div className="md:hidden bg-black/80 backdrop-blur-md text-white uppercase text-sm tracking-wider px-6 py-4 space-y-4">
//           {/* Anchor link to Home */}
//           <a
//             href="#home"
//             className="block hover:text-yellow-400"
//             onClick={() => setMenuOpen(false)}
//           >
//             Home
//           </a>

//           {/* ✅ Members link opens in a NEW TAB (CHANGED) */}
//           <a
//             href="/members"
//             target="_blank" // Opens in new tab
//             rel="noopener noreferrer"
//             className="block hover:text-yellow-400"
//             onClick={() => setMenuOpen(false)}
//           >
//             Members
//           </a>

//           {/* Other anchor links */}
//           <a
//             href="#skills"
//             className="block hover:text-yellow-400"
//             onClick={() => setMenuOpen(false)}
//           >
//             Events
//           </a>
//           <a
//             href="#projects"
//             className="block hover:text-yellow-400"
//             onClick={() => setMenuOpen(false)}
//           >
//             Blogs
//           </a>
//           <a
//             href="#connect"
//             className="block hover:text-yellow-400"
//             onClick={() => setMenuOpen(false)}
//           >
//             Connect
//           </a>

//           {/* Resume/Join Us button in mobile menu */}
//           <button
//             onClick={() => {
//               handleResumeDownload();
//               setMenuOpen(false);
//             }}
//             className="bg-gray-200 text-black px-5 py-1 rounded-full font-semibold text-sm hover:bg-gray-300 transition w-full"
//           >
//             Resume
//           </button>
//         </div>
//       )}
//     </nav>
//   );
// };

// export default Navbar;
